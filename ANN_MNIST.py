...
valid_img = file_trans_np(train_img_path,'img')[train_num:train_num+valid_num]
...
def accuracy(parameters, types, top_n=1):
    correct = 0
    for img_i in range(len(train_img) if types == "train" else
                       len(valid_img) if types == "valid" else len(test_img)):
        # 选择相应的数据集
        if types == "train":
            img = train_img[img_i]
            lab = train_lab[img_i]
        ...
        if top_n == 1:
            ...
            # Top-5: 判断前5个预测类别中是否包含真实标签
            top_5_preds = pred.argsort()[-5:][::-1]  # 获取前5个最大值的索引
            if lab in top_5_preds:
                correct += 1

    # 返回准确率
    return correct / len(train_img) if types == "train" else \
           correct / len(valid_img) if types == "valid" else \
...
print(accuracy(parameters, "test", 1))
print(accuracy(parameters, "test", 5))
...
def confusion_matrix_show(parameters, img_type=test_img, img_num=test_num):
    ...
    for img_i in range(img_num):
        true_labels.append(test_lab[img_i])  # 获取真实标签
        predicted_labels.append(predict(img_type[img_i], parameters).argmax())  # 获取预测标签
